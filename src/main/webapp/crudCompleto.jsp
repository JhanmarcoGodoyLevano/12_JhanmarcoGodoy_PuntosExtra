...
<title>CRUD Person Activos</title>
<link
...
<body
	...
	<div
		...
		style="z-index: 9999; top: 50px; left: 50%; transform: translateX(-50%);"
		role="alert" id="alertaActualizar">
	...
	<div
		class="alert alert-success alert-dismissible fade show position-fixed"
		style="z-index: 9999; top: 50px; left: 50%; transform: translateX(-50%);"
		role="alert" id="alertaReactivado">
			...
			<div
				...
				<div class="bg-danger p-2 align-items-center">
					...
					<ul class="nav nav-pills flex-column mt-4">
						...
						<li class="nav-item py-2 py-sm-0 align-items-center"><a
							href="ActualizarAula.jsp"
							class="nav-link text-white text-center "> <span
								class="fs-4 ms-3 d-none d-sm-inline">ACTUALIZAR AULA</span>
						</a></li>
					</ul>
				</div>
			</div>
							...
							<div class="container mx-auto text-center">
								...
								<div class="modal fade" id="exampleModal" tabindex="-1"
									...
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												<h1 class="modal-title fs-5" id="exampleModalLabel">Subir
													archivo Excel</h1>
											...
											<div class="modal-body">
												<form action="Controlador" method="POST"
													...
													<div class="form-group">
														<label for="archivoExcel">Selecciona un archivo
															Excel:</label> <input type="file" class="form-control-file"
															id="archivoExcel" name="archivoExcel">
													</div>
													<div class="modal-footer">
														<button type="button" class="btn btn-danger"
															data-bs-dismiss="modal">Cerrar</button>
														<button type="submit" class="btn btn-success"
															name="accion" value="Subir">Subir</button>
													</div>
												</form>
											</div>
										</div>
...
descargarCSVBtn.addEventListener('click', function () {
  ...
  const table = document.getElementById('tablaPerson');
  const data = Array.from(table.rows).slice(1).map(function (row) {
    return {
      ...
      'GENERO': row.cells[3].textContent,
      'NOMBRE': row.cells[4].textContent,
      'APELLIDO': row.cells[5].textContent,
      'NACIMIENTO': row.cells[6].textContent,
      'EMAIL': row.cells[7].textContent,
      'TELEFONO': row.cells[8].textContent,
      'UBIGEO': row.cells[9].textContent,
    };
  });

  const filename = 'Listado_Person_Activos.csv';
  ...
  descargarExcelBtn.addEventListener('click', function() {
    ...
    // Llamamos a la tabla y colocamos los datos
    var table = document.getElementById('tablaPerson');
    ...
    var filename = 'Listado_Person_Activos.xlsx';
  ...
  botonDescargarPDF.addEventListener('click', () => {
    const tablaEstudiantes = document.getElementById('tablaPerson');

    // Configura las opciones de html2pdf
    const options = {
      filename: 'Listado_Person_Activos.pdf',
