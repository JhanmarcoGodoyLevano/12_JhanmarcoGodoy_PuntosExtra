...
<title>CRUD Person Eliminados</title>
<link
	...
	<div class="container-fluid">
		<div class="row flex-nowrap">
			...
			<div class="col p-0">
				...
				<div class="container">
					<h2>Personas encontradas: ${personList.size()}</h2>
						...
						<table class="table table-bordered" id="tablaPerson">
							...
							<tbody>
								<c:forEach var="person" items="${personList}">
									<tr>
										<%-- 						<td>${person.getId()}</td> --%>
										<td>${person.getDocument_type()}</td>
										<td>${person.getDocument_number()}</td>
										<%-- 						<td>${person.getRole()}</td> --%>
										<%-- 						<td>${person.getStatus()}</td> --%>
										<td>${person.getTurnDB()}</td>
										<td>${person.getNames()}</td>
										<td>${person.getLast_names()}</td>
										<td>${person.getFormattedBirthdate()}</td>
										<td>${person.getGenderDB()}</td>
										<td>${person.getEmail()}</td>
										<td>${person.getPhone()}</td>
										<td>${person.getCityFromUbigeo()}</td>
										<td>
...
descargarCSVBtn.addEventListener('click', function () {
  ...
  const filename = 'personListadorPor_NombreApellidos_Eliminados.csv';
  ...
  descargarExcelBtn.addEventListener('click', function() {
    ...
    var filename = 'personListadorPor_NombreApellidos_Eliminados.xlsx';
  ...
  botonDescargarPDF.addEventListener('click', () => {
    ...
    const options = {
      filename: 'personListadorPor_NombreApellidos_Eliminados.pdf',
